...
def build_samples(number_of_generating_samples, desired_sum_of_components, setting_of_generation_path):
    setting_of_generation = pd.read_csv(setting_of_generation_path, index_col=0, header=0)
    ...
    x_generated.to_csv('./../data/candidate/generated_samples.csv')  # 生成したサンプルをcsv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
...
def select_samples(number_of_selecting_samples,number_of_random_searches,generated_samples_path):
    ...
    # 保存
    selected_samples.to_csv('./../data/candidate/selected_samples.csv')  # 選択されたサンプルを csv ファイルに保存。
    remaining_samples.to_csv('./../data/candidate/remaining_samples.csv')  # 選択されなかったサンプルを csv ファイルに保存。
